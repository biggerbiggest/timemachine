...
class projectSpider(base_spider.Spider):
    name = 'yn_xsbn_project'
    source = '西双版纳州公共资源交易网'
    ch_area = '云南省'
    ch_city = '西双版纳州'
    ch_region = '西双版纳州'
    en_area = 'yunnan'
    en_city = 'xishuangbanna'
    en_region = 'xishuangbanna'
    url = 'http://www.xsbnggzyjyxx.com/jyxx/jsgcZbjggs'
    type = 0
    headers = {
    ...
    def parse_project(self, response):
        ...
        for i in range(1, all_page+1):
            ...
            tr_list.pop(0)
            for tr in tr_list:
                detail_url = 'http://www.xsbnggzyjyxx.com' + tr.xpath("./td[2]/a/@href")[0]
    ...
    def parse_info(self, response):
        ...
        if ar_name == '无' or ar_name == '/':
            ar_name = ''
        ProjectModel().insert_project(self.name, com_name, ar_name, pro_name, pro_date, money, self.source, original_url, duration,
                                      registration_num, self.ch_area, self.ch_city, self.ch_region,self.en_area, self.en_city,
                                      self.en_region, self.crawler_id, self.spider_id,self.headers['User-agent']
                                      )
